...
export namespace PermissionNext {
  ...
  export const ask = fn(
    ...
    async (input) => {
      ...
      // YOLO mode: auto-approve everything except explicit denies
      if (Yolo.isEnabled()) {
        for (const pattern of request.patterns ?? []) {
          const rule = evaluate(request.permission, pattern, ruleset, s.approved)
          // Still respect explicit deny rules even in YOLO mode
          if (rule.action === "deny") {
            throw new DeniedError(ruleset.filter((r) => Wildcard.match(request.permission, r.permission)))
          }
        }
        log.info("yolo mode: auto-approved", { permission: request.permission, patterns: request.patterns })
        return
      }
